let centers = [];  // centre empty array 
const cards = document.querySelector(".cards");  //returns the first element that matches a specified CSS.
const searchBtn = document.querySelector(".searchBox").querySelector("button");

let today, d, m, y;  //var 
today = new Date();  //date method
d = today.getDate();        
m = today.getMonth() + 1;
y = today.getFullYear();
today = `${d}-${m}-${y}`;
...
function cowinData(pincode) {
  ...
  xhr.onload = function () {
    if (this.status === 200) {
      ...
      if(data.sessions !== []){     // checking the sesssion array is empty or not 
        data.sessions.map((e, i) => {    // map function getting element and index array
          let centerInfo = [   // centre info array 
            e.name,
            e.address,
            e.vaccine,
            e.date,
  ...
  <h3>
    <span class="category">Available Slots - </span>
     ${centers[i][8].join(" | ")}
          ...
          cards.innerHTML += code;  // returns html content 
        });
...
const input = document.querySelector("#input")  //searchbox for inpit box 
